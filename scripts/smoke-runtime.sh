...
PROJECT_NAME="${COMPOSE_PROJECT_NAME:-maternity-care-runtime-smoke}"
...
for service in postgres backend admin-web desktop-web; do
  wait_for_healthy "$service"
done

for attempt in $(seq 1 60); do
  if curl -fsS http://localhost:8080/client-api/v1/health >/dev/null; then
    curl -fsS http://localhost:4174 >/dev/null
    curl -fsS http://localhost:4176 >/dev/null
