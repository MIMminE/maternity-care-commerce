...
- 고객 웹, iOS 앱, 관리자 백오피스, API 서버를 분리한 서비스 실행 단위 설계
- 사용자 API와 관리자 API의 권한 경계, 응답 모델, 감사 기준 분리
- 민감정보, 동의 이력, 상담, 마케팅 대상 조회처럼 운영 리스크가 있는 도메인 모델링
- 주문 당시 가격 스냅샷, 상담 상태, 관리자 조회 감사 로그 등 운영 데이터 무결성 고려
- 백엔드 도메인 흐름을 사용자 경험과 관리자 업무 화면으로 연결하는 구현 방식
...
이 프로젝트는 고객 화면, 모바일 앱, 관리자 화면을 단순히 많이 만든 포트폴리오가 아닙니다. 민감한 고객 정보와 운영 데이터를 다루는 서비스에서 API 경계, 감사, 데이터 무결성을 어떻게 잡을지 보여주는 데 목적이 있습니다.

| 문제 | 설계 대응 |
| --- | --- |
| 고객 경험과 운영자 업무가 같은 데이터를 보지만 접근 목적이 다름 | `/client-api/v1/**`, `/admin-api/v1/**`로 API 경계와 응답 모델 분리 |
| 임신/출산 관련 정보는 민감정보가 될 수 있음 | 케어 프로필, 동의 이력, 관리자 조회 감사 로그를 별도 모델로 관리 |
| 약관/마케팅 동의는 현재값만으로는 운영 대응이 어려움 | `ConsentHistory`로 동의 항목과 시점을 이력화 |
| 상품 가격은 바뀌지만 과거 주문 금액은 흔들리면 안 됨 | `OrderItem.unitPrice`에 주문 당시 가격 스냅샷 저장 |
| 상담/문의 처리는 고객 상태와 관리자 처리 이력이 연결되어야 함 | 상담 상태 변경과 관리자 액션을 운영 화면과 감사 로그로 연결 |
...
| 설계 선택 | 이유 |
| --- | --- |
| 고객 웹, iOS 앱, 관리자 웹 분리 | 사용자 경험, 모바일 반복 사용, 운영자 업무의 화면 밀도와 배포 방식이 다르기 때문에 |
| client/admin API 분리 | 같은 도메인이라도 권한, 응답 모델, 감사 기준이 다르기 때문에 |
| ConsentHistory 모델 | 동의는 현재 상태뿐 아니라 언제 어떤 항목에 동의했는지가 중요하기 때문에 |
| AuditLog 모델 | 운영자의 회원/케어 정보 조회와 상담 변경은 추적 가능한 이벤트여야 하기 때문에 |
| 관리자 역할별 접근 제어 | CS, 운영, 마케팅, 법무 역할이 같은 관리자 API라도 서로 다른 범위만 사용해야 하기 때문에 |
| OpenAPI Bearer 인증 | Swagger UI에서도 JWT를 넣어 보호 API와 권한 경계를 수동 검증할 수 있게 하기 위해 |
| 주문 가격 스냅샷 | 상품 가격 변경이 과거 주문 금액을 훼손하지 않도록 하기 위해 |

이 프로젝트는 “프론트엔드 화면을 많이 만든 백엔드 프로젝트”가 아니라, 고객 경험과 운영 경험을 기준으로 API 경계와 데이터 무결성을 설계한 포트폴리오입니다.
...
검증은 고객 화면, 관리자 화면, iOS 타입 체크, 백엔드 테스트를 나눠 확인했습니다.

| 검증 항목 | 확인 내용 |
| --- | --- |
| 고객 웹 | 상품 탐색, 장바구니, 주문 화면이 빌드 가능한지 확인 |
| 관리자 웹 | 회원, 상품, CS, 마케팅, 감사 로그 화면이 타입 오류 없이 빌드되는지 확인 |
| iOS 앱 | SwiftUI 화면과 모델이 타입 체크를 통과하는지 확인 |
| 백엔드 | client/admin API 경계, 인증, 주문/상담 도메인 테스트가 통과하는지 확인 |
| 포트폴리오 패키지 | 본문, 이미지, 문서, manifest가 S3 feed 구조로 묶이는지 확인 |
...
포트폴리오 허브에는 본문, 이미지, 문서 링크, manifest를 하나의 패키지로 묶어 게시합니다.
...
업로드는 수동 GitHub Actions로만 실행해, 고객/관리자 화면과 문서가 공개 가능한 상태로 정리된 뒤 S3 feed에 반영하도록 했습니다.
