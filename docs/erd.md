...
1차 데이터 모델은 일반 커머스 흐름에 모성 케어 도메인을 얹는 방식으로 설계합니다.

## 핵심 영역

- 회원/산모 프로필: `Member`, `PregnancyProfile`
...
`AuditLog`는 관리자가 회원 정보나 산모 프로필처럼 민감한 정보를 조회하거나 운영 데이터를 내보낼 때 남기는 접근 기록입니다.
