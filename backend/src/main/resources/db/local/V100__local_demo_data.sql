...
values
    ...
    ('marketing@example.com', '이고객', '010-3000-4000', 'ACTIVE', '{noop}password123!', current_timestamp, current_timestamp)
on conflict (email) do nothing;
...
values
    ('케어 바디로션', 'BODY_CARE', 32000.00, 30, 'ON_SALE', current_timestamp, current_timestamp),
    ('케어 샴푸', 'HAIR_CARE', 28000.00, 25, 'ON_SALE', current_timestamp, current_timestamp),
    ('케어 어메니티 세트', 'GIFT_SET', 54000.00, 12, 'ON_SALE', current_timestamp, current_timestamp)
...
from members m
join products p on p.name = '케어 샴푸'
